...
import { fetchStandardFields } from "../api";
import type { StandardField } from "../api";
...
export default function StandardFieldsPage() {
  ...
  // 컬럼명(fieldId)으로 정렬 및 검색 필터링
  const sortedFields = [...standardFields]
    .filter((field) => field && field.fieldId)
    .filter((field) => {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            시스템에서 사용하는 표준 필드를 조회합니다. (총 {sortedFields.length}개)
          </p>
        </div>
      </div>
