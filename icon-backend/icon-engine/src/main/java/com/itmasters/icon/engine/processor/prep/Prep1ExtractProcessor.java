...
public class Prep1ExtractProcessor {
    ...
    /**
     * PREP-1 실행: 데이터 읽기 → 스키마 검증/매핑 → mapped_storages 저장
     *
    ...
    protected Step1Result handleFailure(Long execDsMpId, String errorMessage) {
        log.error("PREP-1 실패 처리 - execDsMpId: {}, error: {}", execDsMpId, errorMessage);
    ...
    private Step1Result finalizeStep(String dataSourceId,
        ...
        ExecDsMpEntity execDsMp = execDsMpRepoitory.findById(execDsMpId)
                .orElseThrow();
        return Step1Result.successWithLanding(execDsMp, savedCount);
    }
